package com.bluewind.boot.module.sys.syspostinfo.controller;
...
public class SysPostInfoController extends BaseController {
    final static Logger logger = LoggerFactory.getLogger(SysPostInfoController.class);
    ...
    public BaseResult list(@RequestParam("page") Integer pageNum,
        ...
        List<SysPostInfo> accounts = sysPostInfoService.getSysPostInfoList(paraMap);

        PageInfo<SysPostInfo> pageinfo = new PageInfo<>(accounts);
        // 取出查询结果
        List<SysPostInfo> rows = pageinfo.getList();
    ...
    public BaseResult add(@RequestParam("postName") String postName,
        ...
        SysPostInfo sysPostInfo = new SysPostInfo();
        sysPostInfo.setPostId(IdGenerate.nextId());
        sysPostInfo.setPostName(postName);
        sysPostInfo.setPostCode(postCode);
        sysPostInfo.setPostSort(postSort);
        sysPostInfo.setStatus(status);
        sysPostInfo.setDescript(descript);
        sysPostInfo.setCreateUser(getSysUserId());

        int num = sysPostInfoService.add(sysPostInfo);
    ...
    public String forUpdate(Model model, @PathVariable String postId) {
        // 获取下拉栏枚举值
        List<Map<String,String>> baseDictList = DictUtils.getDictList("syspostinfo_status");
        SysPostInfo sysPostInfo = sysPostInfoService.getOne(postId);
        model.addAttribute("baseDictList", baseDictList);
        model.addAttribute("sysPostInfo", sysPostInfo);
    ...
    public BaseResult update(@RequestParam("postId") String postId,
        ...
        int num = sysPostInfoService.update(sysPostInfo);
    ...
    public BaseResult delete(@PathVariable String postId,@PathVariable String postName) {
        int num = sysPostInfoService.delete(postId);
        if (num > 0) {
